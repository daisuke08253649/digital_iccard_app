...
## 16. コードレビューワークフロー（Claude Code + CodeRabbit）
...
- **CodeRabbit CLI**: ローカルでのコード品質・セキュリティの即時検証。
...
Claude Code がコードを作成します。完了後、Claude Code 自身に CodeRabbit CLI を実行させます。
\*\*```

# Claude Code に実行させるコマンド

git add .
coderabbit --prompt-only > review.txt
cat review.txt

````**

#### 3. **即時修正**:
review.txt の内容を読み取らせ、Claude Codeに修正を実行させます。(人間が指示)
**```
指示例: "review.txtの指摘をすべて反映し、完了したらファイルを削除してください"
```**

#### 4. **PR作成 & PRレビュー**:
修正完了後、Claude CodeがPRを作成し、GitHub上のCodeRabbitによる最終レビューを受けます。

### Claude Codeへの指示テンプレート

**【ステップ1：ローカル品質検証】**
実装が完了したら、git add した後に coderabbit --prompt-only > review.txt を実行し、その内容を確認してPR作成前にセルフ修正を行ってください。

**【ステップ2：GitHub上の指摘への対応】**
